  ...
  void onSendMessage(String messages)async{
    try{
      if(supabase.auth.currentUser != null){
        final chatRoomId = await supabase.from('chats_ID').insert({
      ...
      body: Center(
        child: Column(
          children: [
            Expanded(
              child: StreamBuilder(
                stream: supabase.from('chats_ID').stream(primaryKey: ['id']).eq('chatRoomID', widget.chatRoomId ),
                        ...
                        itemBuilder: (context,index){
                          ...
                          if(supabase.auth.currentUser != null){
                            ...
                            MessageTile(message: messages[index]['messages'].toString(),
                                sender: messages[index]['created_by'] == supabase.auth.currentUser!.id ? widget.currentUserName :
                                widget.receiverUserMap['name'],
                                sendByMe: messages[index]['created_by'] == supabase.auth.currentUser!.id)



                                :ImageMessage(url: messages[index]['image_url'], sendByMe: messages[index]['created_by'] == supabase.auth.currentUser!.id,
                                sender: messages[index]['created_by'] == supabase.auth.currentUser!.id ? widget.currentUserName:
                                widget.receiverUserMap['name'] );
                          }
                          else{
